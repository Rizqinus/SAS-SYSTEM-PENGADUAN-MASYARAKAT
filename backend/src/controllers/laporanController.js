...
export const createLaporan = async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO laporan (id, pelapor_id, category_id, judul, deskripsi, lokasi_jalan, tingkat_kerusakan, foto_url) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            laporan_id,
            pelapor_id,
            category_id,
...
export const getAllLaporan = async (req, res) => {
    try {
        const [laporan] = await pool.query(`SELECT l.*, u.nama_lengkap as nama_pelapor, c.nama_kategori 
       FROM laporan l 
...
export const getMyLaporan = async (req, res) => {
    ...
    `, [userId]);
        ...
        res.status(500).json({ message: 'Gagal mengambil data laporan saya.', error: error.message });
    }
};
export const getLaporanById = async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.query(`SELECT l.*, u.nama_lengkap as nama_pelapor, c.nama_kategori 
       FROM laporan l 
       JOIN users u ON l.pelapor_id = u.id 
       LEFT JOIN categories c ON l.category_id = c.id
       WHERE l.id = ?`, [id]);
