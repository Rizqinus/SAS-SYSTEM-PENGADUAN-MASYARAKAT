...
export const createKomentar = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [laporan] = await pool.query<RowDataPacket[]>('SELECT id FROM laporan WHERE id = ?', [laporanId]);
    if (laporan.length === 0) {
    ...
    await pool.query(
      'INSERT INTO komentar (id, laporan_id, user_id, isi_komentar) VALUES (?, ?, ?, ?)',
      [komentarId, laporanId, userId, isi_komentar]
    );
    ...
    const [newComment] = await pool.query<RowDataPacket[]>(
      `SELECT k.*, u.nama_lengkap as nama_user, u.role as role_user 
       FROM komentar k 
    ...
    res.status(500).json({ message: 'Gagal menambahkan komentar.', error: (error as Error).message });
  }
};

// Mengambil semua komentar untuk laporan tertentu
export const getKomentarByLaporan = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { laporanId } = req.params;
    ...
    res.status(500).json({ message: 'Gagal mengambil komentar.', error: (error as Error).message });
  }
};

// Menghapus komentar
export const deleteKomentar = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: 'Gagal menghapus komentar.', error: (error as Error).message });
  }
};
