...
app.listen(PORT, async () => {
  ...
  try {
    ...
    const [catCount] = await connection.query<RowDataPacket[]>("SELECT COUNT(*) as count FROM categories");
    ...
    try {
      const [columns] = await connection.query<RowDataPacket[]>("SHOW COLUMNS FROM laporan");
      ...
      if (!columnNames.includes('category_id')) {
        ...
        try {
          ...
          console.log("Foreign Key constraint info:", (fkErr as Error).message);
        }
      ...
      console.error("❌ Gagal melakukan auto-migrasi tabel laporan:", (migError as Error).message);
    }
    ...
    console.error("❌ Gagal terkoneksi ke database MySQL:", (error as Error).message);
  }
});
